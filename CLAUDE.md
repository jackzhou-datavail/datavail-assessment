...
**`patterns/`** is a growing reference library of Databricks patterns and anti-patterns (one markdown file each, anti-patterns flagged right after the title). It's documentation, not deployed code — lives on `master` regardless of what branch the assessment tooling itself is on. See `patterns/README.md`.

**Branches:** `master` has the synthetic-data demo (this file describes it). `real_data` is an isolated variant that extracts real metrics from this actual workspace's Unity Catalog system tables instead of fabricating data — separate bundle name (`datavail-assessment-v2`), separate schema/dashboard/Genie names, so deploying either branch never touches the other's resources.
